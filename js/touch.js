...
function onTouchCancel(event){
    ...
    for(var i = 0; i < event.changedTouches.length; i++){
        ...
        if(sliderTouchIDs[event.changedTouches[i].identifier]){
            delete sliderTouchIDs[event.changedTouches[i].identifier];
            continue;
        }

        var index = findTrackedTouch(event.changedTouches[i].identifier);
        if(index !== -1){
            var touch = touches[index];

            // Save active voice info before cleanup
            var activeVoices = [];
            for(var key in touch.voices){
                if(touch.voices[key] && touch.voices[key].isPlaying){
                    activeVoices.push({
                        parentUUID: key,
                        intersectedBlock: touch.voices[key].intersectedBlock
                    });
                }
            }
            ...
            // Check if this touch is still physically on the screen
            var stillActive = false;
            for(var j = 0; j < event.touches.length; j++){
                if(event.touches[j].identifier === event.changedTouches[i].identifier){
                    stillActive = true;
                    break;
                }
            }

            // If touch is still on screen, recreate it with its voices
            if(stillActive && activeVoices.length > 0){
                let scaledPointer = getScaledPointer(event.changedTouches[i]);
                let raycaster = new THREE.Raycaster();
                raycaster.setFromCamera(scaledPointer, camera);
                ...
                // Restart voices
                for(var k = 0; k < activeVoices.length; k++){
                    var v = new voice();
                    v.playVoice(activeVoices[k].intersectedBlock);
                    trackedTouch.voices[activeVoices[k].parentUUID] = v;
                }
